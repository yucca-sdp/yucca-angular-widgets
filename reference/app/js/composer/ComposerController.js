...
app.controller('ComposerCtrl', ['$scope', '$compile', '$uibModal','readFilePreview','metadataapiAPIservice', '$timeout', 'localStorageService','$location',
	...
	$scope.isEditingGrid = false;
	$scope.toggleEditGrid = function(){
		$scope.isEditingGrid = !$scope.isEditingGrid;
		if(!$scope.isEditingGrid){
			if( $scope.dashboard.grid && $scope.dashboard.grid.panels){
				for(var i=0; i<$scope.dashboard.grid.panels.length; i++){
					$scope.dashboard.grid.panels[i].selected = false
				}				
			}
			$scope.seedGridKey = null;
			$scope.selectedGridPanelCount = 0;
			$scope.currentGridPanel = null;			
		}
	}
...
//	);
	...
	$scope.refreshGridPrevew = function(){
		...
		//$scope.dashboard.panels = ['item_0_0'];
		for (var i = 0; i < $scope.dashboard.components.length; i++) {
			if(!$scope.dashboard.components[i].panelName)
				$scope.dashboard.components[i].panelName='item_0_0';
		}
		var gridNames = "";
		var gridMap = {}
		for(var i=0; i<$scope.dashboard.grid.panels.length; i++){
			if(!gridMap['r_' + $scope.dashboard.grid.panels[i].r])
				gridMap['r_' + $scope.dashboard.grid.panels[i].r] = "";
			gridMap['r_' + $scope.dashboard.grid.panels[i].r] += $scope.dashboard.grid.panels[i].name + " ";
			
			//if(!gridMap['r_' + $scope.dashboard.grid.panels[i].r]['c_' + $scope.dashboard.grid.panels[i].c])
			//	gridMap['r_' + $scope.dashboard.grid.panels[i].r]['c_' + $scope.dashboard.grid.panels[i].c] = "";
		}
		for (var rKey in gridMap) {
		    if (gridMap.hasOwnProperty(rKey) ) {
		    	gridNames += "'" + gridMap[rKey] + "' ";
		    }
		}
		$scope.dashboard.grid.templateAreas = gridNames;
		console.log("gridNames ", gridNames);
		
//		if($scope.dashboard.grid.numRows>0 && $scope.dashboard.grid.numColumns>0){
//			var counter = 1;
//			var gridStyle = "";
//			var gridNames = "";
...
//			
//			for(var i=0; i<$scope.dashboard.grid.numRows; i++){
...
//			}
	...
	$scope.openDashboardConfigDialog = function(){
		...
		modalInstance.result.then(function (dashboard) {
			...
			if(dashboard){
				...
				if($scope.dashboard.widgetPositionType == 'grid' && !$scope.dashboard.grid){
					$scope.dashboard.grid = {panels: [{name:'item_0_0', r:0, c:0}]}
					$scope.dashboard.styleGrid = {gap:0, templateRows:[''], templateColumns: ['']};
					$scope.refreshGridPrevew();
				}
				$scope.refreshDashboardStyle();
			}
	...
	var maxGridRowsColumns = function(){
		var max = [0,0];
		if($scope.dashboard.grid && $scope.dashboard.grid.panels){
			for(var i=0; i<$scope.dashboard.grid.panels.length; i++){
				if($scope.dashboard.grid.panels[i].r>max[0])
					max[0] = $scope.dashboard.grid.panels[i].r;
				if($scope.dashboard.grid.panels[i].c>max[1])
					max[1] = $scope.dashboard.grid.panels[i].c;
			}
		}
		return max;
	}
	...
	$scope.selectGridPanel = function(panelIndex){
		if($scope.isEditingGrid){
			if( $scope.dashboard.grid.panels[panelIndex]){
				$scope.dashboard.grid.panels[panelIndex].selected=!$scope.dashboard.grid.panels[panelIndex].selected;
				
				if($scope.dashboard.grid.panels[panelIndex].selected && $scope.seedGridKey == null )
					$scope.seedGridKey = $scope.dashboard.grid.panels[panelIndex];
				
				if(!$scope.dashboard.grid.panels[panelIndex].selected  && $scope.seedGridKey  && $scope.seedGridKey.name == $scope.dashboard.grid.panels[panelIndex].name)
					$scope.seedGridKey = null;		
				
				$scope.selectedGridPanelCount = 0;
				for(var i=0; i<$scope.dashboard.grid.panels.length; i++){
					if($scope.dashboard.grid.panels[i].selected){
						$scope.selectedGridPanelCount++;
						$scope.currentGridPanel = $scope.dashboard.grid.panels[i];
					}
					
				}
				if($scope.selectedGridPanelCount!=1)
					$scope.currentGridPanel = null;
	...
	$scope.gridPanelStyle = function(panelIndex){
		return $scope.dashboard.grid.panels[panelIndex].selected?"selected":"";
	};
		
	$scope.groupGridPanels = function(){
		if($scope.seedGridKey){
			for(var i=0; i<$scope.dashboard.grid.panels.length; i++){
				if($scope.dashboard.grid.panels[i].selected && $scope.dashboard.grid.panels[i].name!=$scope.seedGridKey.name){
					for(var c=0; c<$scope.dashboard.components.length; c++){
						if($scope.dashboard.components[c].panelName == $scope.dashboard.grid.panels[i].name)
							$scope.dashboard.components[c].panelName = $scope.seedGridKey.name;
					}
					
					$scope.dashboard.grid.panels[i].name = $scope.seedGridKey.name;
					$scope.dashboard.grid.panels[i].deleted = true;
				}
				$scope.dashboard.grid.panels[i].selected = false;
			}
	...
	$scope.ungroupGridPanels = function(){
		
		for(var i=0; i<$scope.dashboard.grid.panels.length; i++){
			if($scope.dashboard.grid.panels[i].selected){
				$scope.dashboard.grid.panels[i].name = "item_" + $scope.dashboard.grid.panels[i].r + "_" + $scope.dashboard.grid.panels[i].c;
				$scope.dashboard.grid.panels[i].deleted = false;
			}
			$scope.dashboard.grid.panels[i].selected = false;
		}
		$scope.seedGridKey = null;
	...
	$scope.resetGridGroupPanels = function(){
		for(var i=0; i<$scope.dashboard.grid.panels.length; i++){
			$scope.dashboard.grid.panels[i].name = "item_" + $scope.dashboard.grid.panels[i].r + "_" + $scope.dashboard.grid.panels[i].c;
		}		
		$scope.seedGridKey = null;
	...
	var maxGridColumns = function(row){
		var max = 0;
		if($scope.dashboard.grid && $scope.dashboard.grid.panels){
			for(var i=0; i<$scope.dashboard.grid.panels.length; i++){
				
				if($scope.dashboard.grid.panels[i].r==row && $scope.dashboard.grid.panels[i].c>max)
					max = $scope.dashboard.grid.panels[i].c;
			}
		}
		return max;
	}
	...
	$scope.addGridRow = function(){
		var rowscols = maxGridRowsColumns();
		var r =  rowscols[0]+1
		for(var i=0; i<rowscols[1]+1; i++){
			$scope.dashboard.grid.panels.push({name:'item_'+r+'_' + i, r:r, c:i});
		}
		$scope.dashboard.styleGrid.templateRows.push('');
		$scope.refreshGridPrevew();
	}
	...
	$scope.addGridColumn = function(){
		var rowscols = maxGridRowsColumns();
		var c = rowscols[1]+1
		for(var i=0; i<rowscols[0]+1; i++){
			$scope.dashboard.grid.panels.push({name:'item_'+i+'_'+c, r:i, c:c});
		}
		$scope.dashboard.styleGrid.templateColumns.push('');
		$scope.refreshGridPrevew();

	}


	$scope.removeGridRow = function(){
		
		var rowscols = maxGridRowsColumns();
		var r =  rowscols[0]
		if(r>0){
			var i = $scope.dashboard.grid.panels.length
			while (i--) {
				if($scope.dashboard.grid.panels[i].r == r){
					for(var c=0; c<$scope.dashboard.components.length; c++){
						if($scope.dashboard.components[c].panelName == $scope.dashboard.grid.panels[i].name)
							$scope.dashboard.components[c].panelName = 'item_0_0';
					}
					$scope.dashboard.grid.panels.splice(i, 1);
				}
			}
			$scope.dashboard.styleGrid.templateRows.pop();
			$scope.refreshGridPrevew();
			
		}
	}

	$scope.removeGridColumn = function(){
		var rowscols = maxGridRowsColumns();
		var col =  rowscols[1]
		if(col>0){
			var i = $scope.dashboard.grid.panels.length
			while (i--) {
				if($scope.dashboard.grid.panels[i].c == col){
					for(var c=0; c<$scope.dashboard.components.length; c++){
						if($scope.dashboard.components[c].panelName == $scope.dashboard.grid.panels[i].name)
							$scope.dashboard.components[c].panelName = 'item_0_0';
					}
			...
			$scope.dashboard.styleGrid.templateColumns.pop();
			$scope.refreshGridPrevew();
	...
	$scope.refreshDashboardStyle = function(){
		var style = "";
		...
		if($scope.dashboard.widgetPositionType == "absolute"){
			...
			if($scope.dashboard.widgetPositionType == "grid"){
				if($scope.dashboard.styleGrid){
					var completeRow = new Array();
					for (var i = 0; i < $scope.dashboard.styleGrid.templateRows.length; i++) {
						var s = $scope.dashboard.styleGrid.templateRows[i];
						completeRow.push(s?s + "fr":"auto");
					}
					style += "grid-template-rows:" + completeRow.join(" ") + ";";
					
					var completeColumn = new Array();
					for (var i = 0; i < $scope.dashboard.styleGrid.templateColumns.length; i++) {
						var s = $scope.dashboard.styleGrid.templateColumns[i];
						completeColumn.push(s?s + "fr":"auto");
					}
					style += "grid-template-columns:" + completeColumn.join(" ") + ";";
					
					if($scope.dashboard.styleGrid.gap)
						style += "display: grid;grid-gap:" + $scope.dashboard.styleGrid.gap + "px;";
					else
						style += "display: grid;grid-gap:2px;";
				}
//				if($scope.dashboard.styleGrid.panel){
//					for (var pKey in $scope.dashboard.styleGrid.panel) {
//					    if ($scope.dashboard.styleGrid.panel.hasOwnProperty(pKey) ) {
//					    	style += "." + pKey + ".grid-panel{";
//							for (var k in $scope.dashboard.styleGrid.panel[pKey]) {
//							    if ($scope.dashboard.styleGrid.panel[pKey].hasOwnProperty(k)) {
//							    	style += k + ":" + $scope.dashboard.styleGrid.panel[pKey][k] + ";";
//							    }
//							}
//							style += "}";
//					    }
...
//				}
	...
	$scope.getDashboardCss = function(prefix){
		if(!prefix)
			prefix = "#composer-content #content";
		var css = "";
		if($scope.dashboard){
			//console.log("getDashboardCss",$scope.dashboard);
			if($scope.dashboard.styleGlobal)
				css += $scope.dashboard.styleGlobal; 
			css += prefix + "{" + $scope.dashboard.style + "}";
			if($scope.dashboard.widgetPositionType == "grid")
				css += prefix + "{grid-template-areas:" + $scope.dashboard.grid.templateAreas + "}";
...
//		];
	...
	$scope.onDropComponentGrid = function(p, $data,$event){
		console.log("onDropComponentGrid",p, $data, $event);
		$scope.addWidget($data, $scope.currentElementCoordinates.x, $scope.currentElementCoordinates.y, p.name);
	}
	...
	$scope.addWidget = function($data, x, y, gridName){
		...
		if($scope.dragdrop.isDragging){
			if($data &&  typeof $data.componentId == 'undefined'){ // new component
				...
				if($scope.dashboard.widgetPositionType == "grid"){
					newComponent.panelName = gridName;
//					for (var i = 0; i < $scope.dashboard.components.length; i++) {
...
//					}
				...
				if($scope.dashboard.widgetPositionType == "grid"){
					$scope.dashboard.components[index].panelName = gridName;
				}
...
//		return out;
	...
	$scope.loadDashboard = function($file){
		...
		readFilePreview.readTextFile($file[0], 10000000, 'UTF-8').then(function(contents){
	...
	$scope.downloadProjectLoading = false;
	$scope.downloadProject = function(){
		$scope.downloadProjectLoading = true;
...
//		});
	...
	var exportDashboardHtml = function(){
		...
		if($scope.dashboard.widgetPositionType == "grid"){
			for (var i = 0; i < $scope.dashboard.grid.panels.length; i++) {
				var panel = $scope.dashboard.grid.panels[i];
				if(!panel.deleted){
					out +=  '<div class="yucca-dashboard-grid-panel ' + panel.name + '" style="grid-area:' + panel.name + '">\n';
					for (var j = 0; j < $scope.dashboard.components.length; j++) {
						var  c = $scope.dashboard.components[j];
						if(c.panelName==panel.name ){
							out += Utils.widget.getWidgetCodeRaw(c);
						}
					}
					out +=  '</div>';
				}
...
//				out += '</div>\n';
	...
	var exportDashboardCss = function(){
		var out="";
		out += $scope.getDashboardCss("#yucca-dashboard");
...
app.controller('GeojsonsDialogInstanceCtrl', function ($scope, $uibModalInstance,p) {
	...
	$scope.setNewGeojson = function(url){
		if(!$scope.newGeojson)
			$scope.newGeojson = {}
		$scope.newGeojson.url = url;
	}
...
app.controller('DashboardConfigDialogInstanceCtrl', function ($scope, $uibModalInstance, extAPIservice, dashboard) {
	...
	$scope.chooseWidgetPosition = function(){
		console.log("chooseWidgetPosition", )
		if($scope.dashboard.widgetPositionType == 'grid'){
			if(!$scope.dashboard.grid.numColumn)
				$scope.dashboard.grid.numColumn = 1;
			if(!$scope.dashboard.grid.numRow)
				$scope.dashboard.grid.numRow = 1;
			$scope.refreshGridPrevew();
		}
	}
...
//	}
	...
	var operators_boolean = [{"value":"eq", "label":"equals", "valueDelimiter": "%27", "isFunction": false, "isDate": false},
        {"value":"ne", "label":"not equals", "valueDelimiter": "%27", "isFunction": false, "isDate": false}];
...
app.controller('DatasetBoxplotColumnsParamDialogInstanceCtrl', function ($scope,metadataapiAPIservice, $uibModalInstance,$translate, p, current, $uibModal ){
	console.log("p" ,current);	 
	$scope.p = p;
	if(p.values){
		$scope.boxplotColumn = $scope.$eval(p.values);
	}
...
//	
	$scope.ok = function () {
		$scope.p.values = JSON.stringify($scope.boxplotColumn);
		console.log("$scope.p.values", $scope.p.values)
...
app.controller('DatasetHeatmapgradientParamDialogInstanceCtrl', function ($scope,$uibModalInstance,$translate, p, current, $uibModal ){
	console.log("p" ,current);	 
	$scope.p = p;
	$scope.orderColor = function(){
		$scope.steps.sort( function(a,b){return a.key < b.key?-1:1;});
	}

	if(p.values){
		var heatmapgradient = $scope.$eval(p.values);
		$scope.steps = [];
		for(var key in heatmapgradient){
			if(heatmapgradient.hasOwnProperty(key)){
				$scope.steps.push({key: key*100, value:heatmapgradient[key]});
			}
		}
		$scope.orderColor();
	} else {
		$scope.steps = [{key: 40, value: '#0000ff'},
				{key: 60, value: '#00ffff'},
				{key: 70, value: '#00ff00'},
				{key: 80, value: '#ffff00'},
				{key: 100, value:'#ff0000'}]
	}
	
	$scope.addColor = function(){
		$scope.steps.push({key:100, value: '#000000'});
	}
	$scope.removeColor = function(stepIndex){
	...
	$scope.ok = function () {
		...
		for (var i = 0; i < $scope.steps.length; i++) {
			heatmapgradient[$scope.steps[i].key/100] = $scope.steps[i].value;
		}
		$scope.p.values = JSON.stringify(heatmapgradient);
		console.log("$scope.p.values", $scope.p.values)
