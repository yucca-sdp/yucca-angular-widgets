var translations_it = {
	...
	/* Boxplotchart*/
	basicDatasetBoxplotChart:'Grafico Boxplot',
	help_intro_basicDatasetBoxplotChart:'Con questo widget è possibile rappresentare la distribuzione di un campione tramite semplici indici di dispersione e di posizione.',
	help_datasource_basicDatasetBoxplotChart:'Sono previste due tipologie di dataset<br><strong>Raggruppamento per colonne</strong><br>\
    	Le serie vengono identificate in base al raggruppamento dei singoli valori presenti nella colonna indicata nel paramentro "Colonna su cui raggruppare", \
		e le statistiche vengono calcolate in base ai valori contenuti nella colonna indicata dal parametro "Colonna con il valore".<br><br><strong>Statistiche già calcolate</strong><br>\
    	I valori delle statistiche vengono recuperati direttamente da colonne specifiche, una colonna per valore',
	help_datasource_example_basicDatasetBoxplotChart:'<p><strong>Raggruppamento per valori</strong><br></p>\
    	<table class="table help-widget-table-example"><thead>\
    	<tr class="params-hint"><th>Colonna su cui raggruppare</th><th colspan="2">Colonne ignorate</th><th>Colonna con i valori</th></tr>\
    	<tr><th>Corso di studio</th><th>Nome</th><th>Cognome</th><th>Età</th></tr></thead>\
    	<tbody><tr><td><span class="">Maths</span></td><td>Moria</td><td>Driver</td><td>21</td></tr>\
    		<tr><td><span class="">Maths</span></td><td>Hallam</td><td>Cobb</td><td>18</td></tr>\
    		<tr><td><span class="">Economics</span></td></td><td>MacMaster</td><td>Cassandre</td><td>43</td></tr>\
    	</tbody></table>\
    		<p><strong>Serie da valori raggruppamento</strong><br></p>\
        	<table class="table help-widget-table-example"><thead>\
        	<tr class="params-hint"><th colspan="5">Colonne con i valori statistici</th></tr>\
        	<tr><th>Course</th><th>Q1</th><th>Q2</th><th>Q3</th><th>Whisker low</th><th>Whiskerhigh</th><th>Outliers</th><th>Outliers Separator</th></th></tr></thead>\
        	<tbody><tr><td>Math</td><td>21</td><td>26</td><td>30</td><td>31</td><td>12</td><td>46;55</td><td>;</td></tr>\
					<tr><td>Economics</td><td>20</td><td>24</td><td>28</td><td>32</td><td>15</td><td>40;52</td><td>;</td></tr>\
					<tr><td>Physics</td><td>19</td><td>25</td><td>27</td><td>30</td><td>16</td><td>41;45</td><td>;</td></tr>\
        	</tbody></table>',
	help_listen_events_basicDatasetBoxplotChart:'<ul><li>Cambio colonna da contare</li><li>Cambio colonna raggruppamento</li></ul>',
	help_sent_events_basicDatasetBoxplotChart:'<ul><li>Navigazione dei dati</li></ul>',
	help_related_widget_basicDatasetBoxplotChart:'Questo widget non rilancia eventi',
	...
	<tbody><tr><td>2008</td><td>BI</td><td>SETTORE EXTRALBERGHIERO</td><td>642</td></tr>\
		...
		/* basicDatasetBubblesMapChart */
		basicDatasetBubblesMapChart: 'Mappa con bolle',
		help_intro_basicDatasetBubblesMapChart:'Con questo widget si rappresenta mappa tematica in cui sono presenti cerchi colorati geolocalizzati con raggio in proporzione al risultato della statistica calcolata',
		help_datasource_basicDatasetBubblesMapChart: 'Il widget si aspetta un dataset che abbia una colonna con il valore che guiderà il raggio della bolla ("Colonna con il valore"), \
			due colonne con le coordinate geografiche del centro della bolla ("Colonna latitudine" e "Colonna longitudine") e una colonna con l\'etichetta che descrive il valore della bolla ("Colonna con l\'etichetta"). \
			Nel parametro "Geojson da utilizzare" si dovrà specificare il file geojson da utilizzare per rappresentare la mappa. Nel file geojson ogni area \
			della mappa è indicata da un elemento di tipo Feature, che ha come attributi, le coordinate del poligono da rappresentare \
			A seconda del file Geojson utilizzato si potrà specificare anche la proiezione geografica adatta ("Proiezioni geografica")',
		help_datasource_example_basicDatasetBubblesMapChart: '<table class="table help-widget-table-example"><thead>\
			<tr class="params-hint"><th>Colonna ignorata</th><th>Colonna con valori</th><th>Colonna latitudine</th><th>Colonna longitudine</th></tr>\
			<tr><th>City</th><th><span class="help-widget-serie-1">Popolazione</span></th><th>Latitudine</th><th>Longitudine</th></tr></thead>\
		<tbody><tr><td>Tokyo</td><td>35.676.000</td><td>35,685</td><td>139,7514</td></tr>\
		       <tr><td>Singapore</td><td>5.183.700</td><td>1,293</td><td>103,8558</td></tr>\
			   <tr><td>Sydney</td><td>4.630.000</td><td>-33,92</td><td>151,1852</td></tr>\
			</tbody></table>',
			help_listen_events_basicDatasetBubblesMapChart: '<ul><li>Cambio colonna su cui raggruppare</li><li>Cambio colonna su cui contare</li><li>Cambio criteri di filtro oData</li></ul>',
			help_sent_events_basicDatasetBubblesMapChart: '<ul><li>Evidenziazione di un\'area della mappa</li><li>Cambio colonna su cui raggruppare</li></ul>',
			help_related_widget_basicDatasetBubblesMapChart: '<ul><li>Tabella distribuzioni</li><li>Data explorer</li></ul>',

		/* basicDatasetHeatmapMapChart */
		basicDatasetHeatmapMapChart: 'Mappa di calore',
		help_intro_basicDatasetHeatmapMapChart:'Con questo widget si rappresenta mappa in cui sono presente aree colorate geolocalizzate con forma e dimensione in base al risultato delle statistiche calcolate',
		help_datasource_basicDatasetHeatmapMapChart: 'Il widget si aspetta un dataset che abbia una colonna con il valore che guiderà l\'area colorata ("Colonna con il valore"), \
			due colonne con le coordinate geografiche dei valori ("Colonna latitudine" e "Colonna longitudine"). \
			Nel parametro "Geojson da utilizzare" si dovrà specificare il file geojson da utilizzare per rappresentare la mappa. Nel file geojson ogni area \
			della mappa è indicata da un elemento di tipo Feature, che ha come attributi, le coordinate del poligono da rappresentare \
			e almeno un codice per identificare il poligono stesso (ad esempio la sigla di una provincia). Questo codice puo\' venir visualizzato passando con il mouse sull\'area \
			A seconda del file Geojson utilizzato si potrà specificare anche la proiezione geografica adatta ("Proiezioni geografica")',
		help_datasource_example_basicDatasetHeatmapMapChart: '<table class="table help-widget-table-example"><thead>\
			<tr class="params-hint"><th>Colonna etichette</th><th>Colonna con valori</th><th>Colonna latitudine</th><th>Colonna longitudine</th></tr>\
			<tr><th>City</th><th><span class="help-widget-serie-1">Popolazione</span></th><th>Latitudine</th><th>Longitudine</th></tr></thead>\
		<tbody><tr><td>Tokyo</td><td>35.676.000</td><td>35,685</td><td>139,7514</td></tr>\
		       <tr><td>Singapore</td><td>5.183.700</td><td>1,293</td><td>103,8558</td></tr>\
			   <tr><td>Sydney</td><td>4.630.000</td><td>-33,92</td><td>151,1852</td></tr>\
			</tbody></table>',
			help_listen_events_basicDatasetHeatmapMapChart: '<ul><li>Cambio colonna su cui raggruppare</li><li>Cambio colonna su cui contare</li><li>Cambio criteri di filtro oData</li></ul>',
			help_sent_events_basicDatasetHeatmapMapChart: '<ul><li>Evidenziazione di un\'area della mappa</li><li>Cambio colonna su cui raggruppare</li></ul>',
			help_related_widget_basicDatasetHeatmapMapChart: '<ul><li>Tabella distribuzioni</li><li>Data explorer</li></ul>',
	...
	help_inline_group_by_datetime: 'Selezionare se la colonna su cui raggruppare è una data',
	...
	help_inline_api_metadata_url: 'Url base da usare per caricare i metadati. Se lasciata vuota i metadati vengono caricati direttamente chiamando le api odata di Yucca. \
		L\'utilizzo di una componente server che fa da tramite fra la dashboard e le api dei metadati è utile quando i dati sono privati e si vuole utilizzare il token di sicurezza. \
		Il token infatti va utilizzato sulla parte server: la dasboard invoca la chiamata alle api senza token, il componente server inietta il token.\
		E\' possibile indicare anche una url relativa, ovvero <code>/componente_server/</code>. La cosa importante è che la url termini con uno <code>/</code>\
		perchè il widget concatenerà il codice del dataset alla url.\
		Nell\'esportazione della dashboard come applicazione java o php saranno presenti anche i sorgenti nei due linguaggi, che possono essere utili per sviluppi custom',
	help_inline_api_data_url: 'Url base da usare per caricare i dati. Se lasciata vuota i dati vengono caricati direttamente chiamando le api odata di Yucca. \
			L\'utilizzo di una componente server che fa da tramite fra la dashboard e le api odata è utile quando i dati sono privati e si vuole utilizzare il token di sicurezza. \
			Il token infatti va utilizzato sulla parte server: la dasboard invoca la chiamata alle api senza token, il componente server inietta il token.\
			E\' possibile indicare anche una url relativa, ovvero <code>/componente_server/</code>. La cosa importante è che la url termini con uno <code>/</code>\
			perchè il widget concatenerà il codice del dataset alla url.\
			Nell\'esportazione della dashboard come applicazione java o php saranno presenti anche i sorgenti nei due linguaggi, che possono essere utili per sviluppi custom',

	help_inline_cache: 'Indica se utilizzare la cache nelle chiamate alle api odata. Utile quando nella stessa applicazione si usa sempre lo stesso dataset. \
		La cache è attiva sulla singola sessione, ricaricando completamente la pagina vengono ricaricati anche i dati',
	...
	help_inline_lat_column: 'Chiave della colonna del dataset con la latitudine del dato',
	help_inline_lng_column: 'Chive della colonna del dataset con la longitudine del dato',
	...
	param_group_by_datetime: 'Colonna su cui raggruppare di tipo data', 
	...
	param_lat_column: 'Colonna latitudine',
	param_lng_column: 'Colonna longitudine',
	...
	param_area_color: 'Colore area geografica',
	help_inline_area_color: 'Colore da usare per le regioni della mappa',

	param_highlight_color: 'Colore confini evidenziati',
	help_inline_highlight_color: 'Colore da usare per i confini quando si passa con in mouse sopra e vengono visualizzate le informazioni dell\'area',
	...
	param_is_data_aggregate:'Dati aggregati',
	param_boxplot_columns: 'Colonne statistiche boxplot',
	param_config_boxplot_q1_column: '1° Quartile (Q1)',
	param_config_boxplot_q2_column: 'Mediana (Q2)',
	param_config_boxplot_q3_column: '3° Quartile (Q3)',
	param_config_boxplot_whisker_low_column: 'Baffo inferiore (Whisker low)',
	param_config_boxplot_whisker_high_column: 'Baffo superiore (Whisker high)',
	param_config_boxplot_outliers_column: 'Osservazioni eccezionali (outliers)',
	param_config_boxplot_outliers_separator_column: 'Separatore per le osservazioni eccezionali',
	
	dataset_boxplot_columns_param_dialog_title: 'Colonne per Boxplot',
	dataset_boxplot_columns_param_dialog_intro: 'Indicare le colonne in cui si trovano i valori statistici calcolati',
	
	
	dataset_heatmap_gradient_param_dialog_title: 'Configurazione colori mappa di calore',
	dataset_heatmap_gradient_param_dialog_intro: 'Indicare i colori che si intende usare, quindi associarli a un valore percentuale di soglia',

	dataset_heatmap_gradient_param_remove_step: 'Rimuovi',
	dataset_heatmap_gradient_param_add_step: 'Aggiungi un colore',
	

	param_radius: 'Raggio',
	param_blur: 'Sfocatura',
	param_heatmap_gradient: 'Gradiente colori',
	help_inline_radius: 'Raggio degli elementi nella mappa',
	help_inline_blur: 'Sfocatura degli elementi nella mappa',
	help_inline_heatmap_gradient: 'Gradiente di colori da utilizzare',
	...
	param_stagger_labels_axis: 'Sfasa etichette assi, una etichetta si e una no viene visualizzata sfasata, utile quando sono etichette lunghe per far in modo che non si sovrappongano', 
	...
	param_api_data_url: 'Url base data api',
	param_api_metadata_url: 'Url base metadata api',
	param_cache: 'Utilizza cache',
	
	param_column_hide: 'Nascondi',
	param_column_format_number: 'Formattaz, numeri',
	param_column_format_date: 'Formattaz. date/stringhe',
	param_column_filter: 'Filtri',
	param_dataexplorer_filter_value:'Filtro da aggiungere in testa alla tabella',
	...
	geojson_piedmont_province: 'Province Piemonte',
	geojson_piedmont_comuni: 'Comuni Piemonte', 
	...
	geojson_url_param_hint: 'Url dove &egrave; presente il file geojson. Il geojson deve risiedere sullo stesso server dove è esposta la webapp',
	geojson_url_param_on_yucca: 'In alternativa scegli fra i geojson presenti su Yucca',
	geojson_url_param_on_yucca_warning: 'I geojson di Yucca sono aggiornati al 2018',
	...
	/* grid */
	dashboard_grid_edit_btn: 'Disegna griglia',
	dashboard_grid_row: 'Riga',
	dashboard_grid_column: 'Colonna',
	dashboard_grid_group: 'Raggruppa',
	dashboard_grid_ungroup: 'Separa',
	dashboard_grid_reset_group: 'Reset',
	dashboard_grid_style_sidebar_title: 'Stile',
	dashboard_grid_grid_style_sidebar_title: 'Griglia',
	dashboard_grid_cell_style_sidebar_title: 'Cella',
	
	dashboard_grid_style_gap:'Spazio fra le celle',
	dashboard_grid_style_gap_help: 'Spazio fra le celle in pixel',
	dashboard_grid_style_template_rows: 'Peso righe',
	dashboard_grid_style_template_columns: 'Peso colonne',
	dashboard_grid_style_template_rows_help: 'Per ogni riga indicare che spazio deve occupare tramite un numero intero. Più è alto il numero più spazio occuperà. \
		Le righe per cui non sarà specificato il numero, occuperanno lo spazio minimo. Le righe con lo stesso numero occuperanno lo stesso spazio. \
		Ad esempio se si hanno 3 righe e si vuole che quella centrale occupi lo spazio massimo, lasciare vuote la riga 1 e 3 e indicare 1 nella riga 2',
	dashboard_grid_style_template_columns_help: 'Per ogni colonna indicare che spazio deve occupare tramite un numero intero. Più è alto il numero più spazio occuperà. \
			Le colonne per cui non sarà specificato il numero, occuperanno lo spazio minimo. Le colonne con lo stesso numero occuperanno lo stesso spazio.\
		Ad esempio se si hanno 3 colonne e si vuole che quella centrale occupi lo spazio massimo, lasciare vuote la colonna 1 e 3 e indicare 1 nella colonna 2',
