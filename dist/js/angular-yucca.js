...
//          
            ...
            if(dataColumns!=null){
            	...
            	metadataService.getDatasetMetadata(tenantcode, datasetcode,user_token).then(
            			function(metadata) {
            				console.log("metadata",metadata);
...
//    '                        <span ng-show="sort.predicate==\'col.key\' && sort.reverse" class="">&darr;</span>'+
    ...
    '               <div class="yucca-widget-dataexplorer-pagination-total">Total <strong>{{tableData.data.length}}</strong></div>' +
    ...
    '			     <a href ng-if="pagination.current < tableData.data.length/pagination.pagesize - 1" ng-click="pagination.current=pagination.current+1" title="Next"  class="yucca-widget-dataexplorer-pagination-prev">&gt;</a>' +
    '   	      </div>\n' +
    '   	    </div>\n' +
...
//		    	   }
        	...
        	var prepareData  = function(){
	    		...
	    		if(odataResult != null){
	    			...
	    			//var colors = $yuccaHelpers.render.safeColors(serieColumns.length, scope.$eval(attr.chartColors),attr.mainChartColor);
	    			scope.chartData = $yuccaHelpers.data.aggregationSeriesXY(allData, groupByColumn, serieColumns, chartColors,attr.mainChartColor);
...
yuccaWidgetsModule.directive('ngYuccaDatasetPieChart', ['metadataService','dataService', '$yuccaHelpers', '$rootScope','$timeout',
    ...
    return {
        ...
        link: function(scope, elem, attr) {
            ...
            scope.chartWidth = $yuccaHelpers.attrs.num(attr.widgettWidth, null, null, null);
            scope.chartHeight = $yuccaHelpers.attrs.num(attr.widgetHeight, null, null, null);
            $timeout(function(){
            	var chartContentElement = angular.element( document.querySelector('#' + scope.widgetId+ " .yucca-widget-chart-content" ));
